...
<div style="background-color:#E0EFF7;">
    <div class="container"> 
        <div class="span-24" style="height:<?php echo $hh; ?>">
            ...
            <?php echo $form->create('Category', array('action' => 'select/'.$id)); ?>
                    ...
                    <tr>
                        ...
                        foreach ($categories as $c) {
                            ...
                            if ($c['Category']['use_ranges']=='0') {
                                echo $form->checkbox('c'.$c['Category']['id'], array('label'=>'','class'=>'checkal','checked'=>true)).'<label> '.$c['Category']['name'].'</label><br/><br/>';
                            } else {
                                $arr = array();
                                foreach ($c['Range'] as $r) {
                                    $arr[$r['id']]=$r['name'];
                                }
                                echo $form->checkbox('c'.$c['Category']['id'], array('label'=>'','class'=>'checkal','checked'=>true)).'<label> '.$c['Category']['name'].'</label><br/>';
                                echo $form->input('v'.$c['Category']['id'], array( 'label' => '','options'=>$arr));
                            }
                            echo '</td>';
                            $cc++;
                        } ?>
