...
foreach ($fields as $f) {
  switch ($f->Field->type) {
    ...
    case 'tinyint':
      ...
      break;
    default:
      echo '<label class="'.$select_label_class.'"></label><input type="text" placeholder="'.strtoupper($f->Field->display_name).'" name="I'.$f->Field->name.'"><br/>';
      break;
  }
}
//echo '<input type="submit" class="'.$submit_button_class.'" value="submit"/></form>';
echo '<br/><div style="width:240px;text-align:center;"><input src="/img/submit.png" type="image"></div>';
